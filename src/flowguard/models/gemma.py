"""Gemma-3-4B-Instruct probe wrapper (multimodal Gemma).

Gemma 3 ships as a unified multimodal transformer. We load via
``Gemma3ForConditionalGeneration`` (transformers >= 4.46).
"""
...
class GemmaProbe(MLLMProbe):
    def __init__(self, model_key: str = "gemma-3-4b", **_) -> None:
    ...
    def probe(
        ...
        p_t = first_token_distribution(self.model, self._build_inputs(None, text), decoding_steps)
        p_v = first_token_distribution(self.model, self._build_inputs(image, neutral_text), decoding_steps)
        p_mm = first_token_distribution(self.model, self._build_inputs(image, text), decoding_steps)
        return ProbeOutputs(
    ...
    @torch.no_grad()
    def generate(self, image, text, max_new_tokens=256, temperature=0.0) -> str:
    ...
    @torch.no_grad()
    def hidden_state(self, image, text) -> np.ndarray:
        inputs = self._build_inputs(image, text)
